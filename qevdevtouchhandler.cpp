...
#define POINT_ID_SPACE          10000

class QEvdevTouchScreenData
{
public:
    QEvdevTouchScreenData(QEvdevTouchScreenHandler *q_ptr, const QStringList &args, int x_offset, int device_id);
    ...
    struct Contact {
        ...
        Contact() : trackingId(-1), pointId(-1),
...
private:
    // Point id generation:
    int m_device_id;
    int m_current_point_id;
};

QEvdevTouchScreenData::QEvdevTouchScreenData(QEvdevTouchScreenHandler *q_ptr, const QStringList &args, int x_offset, int device_id)
    : q(q_ptr),
      m_lastEventType(-1),
      m_currentSlot(0),
    ...
    , m_current_point_id(device_id * POINT_ID_SPACE)
{
...
#endif
{
    ...
    d = new QEvdevTouchScreenData(this, args, x_offset, device_id);
    ...
    qCDebug(qLcEvdevTouch,
            "evdevtouch: %s: Protocol type %c %s (%s), filtered=%s, x offset=%d",
...
void QEvdevTouchScreenData::processInputEvent(input_event *data)
{
    if (data->type == EV_ABS) {
        ...
        if (data->code == ABS_MT_POSITION_X || (m_singleTouch && data->code == ABS_X)) {
            ...
            if (m_typeB) {
                if (m_currentData.trackingId == -1) {
                    ...
                    m_contacts[m_currentSlot].pointId = newPointId();
                }
            }
        ...
        while (it.hasNext())
        {
            ...
            Contact &contact(it.value());
            if (m_typeB) {
                if (contact.state && contact.trackingId != m_contacts[it.key()].trackingId)
                {
                    contact.state = Qt::TouchPointReleased;
                    addTouchPoint(contact, &combinedStates);
                }
            } else {
                if (!m_contacts.contains(contact.trackingId)) {
                    contact.state = Qt::TouchPointReleased;
                    addTouchPoint(contact, &combinedStates);
                }
            }
...
{
    qCDebug(qLcEvdevTouch, "evdevtouch: ASSIGN IDS!!!");
...
void QEvdevTouchScreenData::reportPoints()
{
    ...
    const auto window_width = 1280; // winRect.width()
    const auto window_height= winRect.height();
    ...
    for (int i = 0; i < pointCount; ++i) {
        ...
        const qreal wx = winRect.left() + tp.normalPosition.x() * (window_width - 1) + m_x_offset;
        const qreal wy = winRect.top() + tp.normalPosition.y() * (window_height - 1);
        const qreal sizeRatio = (window_width + window_height) / qreal(hw_w + hw_h);
...
int QEvdevTouchScreenData::newPointId()
{
    ++m_current_point_id;

    if (m_current_point_id >= ((m_device_id + 1) * POINT_ID_SPACE))
    {
        m_current_point_id = m_device_id * POINT_ID_SPACE;
    }

    return m_current_point_id;
}

QEvdevTouchScreenHandlerThread::QEvdevTouchScreenHandlerThread(const QString &device, int x_offset, int device_id, const QString &spec, QObject *parent)
    : QDaemonThread(parent), m_device(device), m_spec(spec), m_x_offset(x_offset), m_device_id(device_id), m_handler(nullptr), m_touchDeviceRegistered(false)
...
{
    m_handler = new QEvdevTouchScreenHandler(m_device, m_device_id, m_spec, m_x_offset);
